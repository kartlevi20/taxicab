...
if ( !function_exists('limme_get_google_fonts') ) {

	function limme_get_google_fonts($css = array()) {
		...
		foreach ( array('font_main', 'font_headers', 'font_subheaders') as $type ) {
			...
			if ( !empty($font['variation']) ) {
				...
				if ( !empty($font_weights) ) {
					...
					$limme_items = explode(',', $font_weights);
					foreach ( $limme_items as $item) {
		...
		if ( !empty($google_fonts) ) {
			...
			wp_enqueue_style( 'limme-google-fonts', esc_url( add_query_arg( $query_args, '//fonts.googleapis.com/css' ) ), array(), null );
		}
