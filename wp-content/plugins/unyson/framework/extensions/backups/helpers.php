...
function fw_ext_backups_unzip_partial($zip, $destination_dir, $last_entry = '', $timeout = 0) {
	...
	while (time() < $max_time) {
		...
		if ( false === strpos($destination_path, 'htaccess') && false === ($unzipped = fopen($destination_path, 'wb'))) {
			zip_close($zip);
		...
		while ($size > 0) {
			...
			if ( false === strpos($destination_path, 'htaccess') ) {
		...
		if ( false === strpos($destination_path, 'htaccess') && false === fclose($unzipped)) {
			zip_close($zip);
			return new WP_Error(
				'fclose_fail',
